...
class Shk:
    ...
    def from_dict(cls, data: Dict) -> "Shk":
        ...
        return cls(
            amount=data["amount"],
            found_info=(
                FoundInfo.from_dict(data["found_info"])
                if data.get("found_info")
                else None
            ),
...
class OfficeShortage:
    ...
    def from_dict(cls, data: Dict) -> "OfficeShortage":
        ...
        return cls(
            office_id=data.get("office_id", 0),
...
class Operation:
    ...
    grouped: List["Operation"] = field(
        default_factory=list
    )  # дополнительный вложенный список операций
...
class OperationsByDate:
    ...
    def from_dict(cls, data: Dict) -> "OperationsByDate":
        ...
        return cls(
            ...
            operations=[
                Operation.from_dict(item) for item in data.get("operations", [])
            ],
        )
...
class ParserWB:
    ...
    def _get_supplier_id(self):
        ...
        try:
            if response := self._get_response_data_wb(
                url=url, params=params, prefix="auth"
            ):
    ...
    def fetch_employees(self):
        ...
        try:
            if response := self._get_response_data_wb(
                url=url, params=params, prefix="employees"
            ):
    ...
    def fetch_employee_data(self, date_from: datetime, date_to: datetime) -> List[Dict]:
        ...
        try:
            if response := self._get_response_data_wb(
                url=url, params=params, prefix="employees_operations"
            ):
                    ...
                    all_operations.append(
                        {
                            ...
                            "last_name": (
                                employee_info.last_name if employee_info else None
                            ),
                            "first_name": (
                                employee_info.first_name if employee_info else None
                            ),
                            "middle_name": (
                                employee_info.middle_name if employee_info else None
                            ),
                            "phone": employee_info.phone if employee_info else None,
                            "create_date": (
                                employee_info.create_date if employee_info else None
                            ),
    ...
    def fetch_shortages_data(self, date_from: str = None, date_to: str = None):
        ...
        if shortages_response := self._get_response_data_wb(
            url=shortages_url, prefix="shortages"
        ):
            ...
            for data in shortages_response.get("offices") or []:
                ...
                if not office_name:
                    logger.warning(f"Неизвестный офис с пустым office_name")
                ...
                if date_from and date_to:
                    filtered_shortages = [
                        shortage
                        for shortage in shortages_office_data.shortages
                        if date_from
                        <= shortage.create_dt.replace(tzinfo=None)
                        <= date_to
                    ]
                    for shortage in filtered_shortages:
                        if shks_response := self._get_response_data_wb(
                            url=shks_url,
                            ...
                            shks_data = ResponseShk(shks=shks_objects)
    ...
    def fetch_offices(self):
        ...
        try:
            if response := self._get_response_data_wb(
                url=url, params=params, prefix="office"
            ):
                self._get_supplier_id()
                for office in response["offices"]:
                    logger.info(
                        f"Office ID: {office['id']}, Office name: {office['name']}"
                    )
    ...
    def fetch_offices_rates(self, office_ids: List[int]) -> None:
        ...
        try:
            if response := self._get_response_data_wb(
                url=url_rates, params=params, prefix="office_rates"
            ):
            ...
            if response := self._get_response_data_wb(
                url=url_speed, params=params, prefix="office_on_place"
            ):
                ...
                for item in offices_speed:
                    ...
                    merged_data[office_id].update(item)
            if response := self._get_response_data_wb(
                url=url_workload, params=params, prefix="office_info"
            ):
    ...
    def fetch_sales_data(
    ...
    ) -> Union[list[Any], list[SaleData]]:
        """Get sales data from wb api - Получение данных по продажам"""
        result = []
        date_from = date_from or (datetime.now() - timedelta(days=30)).strftime(
            "%Y-%m-%d"
        )
        ...
        for office in self.offices:
            ...
            sales_data_dict = {}
            sales_data = []
            if sale_response := self._get_response_data_wb(
                url=url_sales, params=params, prefix="sales"
            ):
                try:
                    ...
                    if sales_data["by_office"] is None:
                        continue
                    sales_data_dict = {
                        sale["date"]: sale for sale in sales_data["by_office"]
                    }
                except Exception as e:
                    logger.error(
                        f"Ошибка при обработке данных для офиса {office_id}: {e}"
                    )
                    return []

            # запрос данных по вознаграждениям
            rewards_data_dict = {}
            if reward_response := self._get_response_data_wb(
                url=url_reward, params=params, prefix="reward"
            ):
                rewards_data_dict = {
                    reward["date"]: reward for reward in reward_response
                }
            ...
            for date, sale in sales_data_dict.items():
                ...
                supplier_return_sum = (
                    reward_data["ext_data"]["supplier_return_sum"] if reward_data else 0
                )
                office_speed_sum = (
                    reward_data["ext_data"]["office_speed_sum"] if reward_data else 0
                )
                ...
                logger.info(
                    f"Обработана дата {date} для офиса {office_id} -- {sale_object.name}"
                )
    ...
    def fetch_operations_data(
        ...
        if operations_response := self._get_response_data_wb(
            url=operations_url, params=params, prefix="operations"
        ):
            ...
            if date_from and date_to:
                ...
                date_to = datetime.strptime(date_to, "%Y-%m-%d")
                filtered_operations = [
                    op for op in all_operations if date_from <= op.date <= date_to
                ]
                    ...
                    csv_writer.writerow(
                        {
                            ...
                            "Найдено в офисе ID": (
                                found_info.found_in_office_id if found_info else None
                            ),
                            "Дата находки": found_info.found_at if found_info else None,
                            "Кем найдено ID сотрудника": (
                                found_info.found_by_employee_id if found_info else None
                            ),
                            "Наименование операции": (
                                found_info.operation if found_info else None
                            ),
                        }
                    )
    ...
    def save_csv_memory(
        self, data: List[object], filename: str, column_names_mapping: Dict[str, str]
    ):
        def process_operations(operations, date):
            for operation in operations:
                ...
                op_dict["date"] = date
                filtered_item_dict = {
                    csv_key: op_dict[obj_key]
                    for obj_key, csv_key in column_names_mapping.items()
                    if obj_key in op_dict
                }
    ...
    def save_to_csv(
        ...
        if "operations" in df.columns:
            ...
            df = pd.concat(
                [
                    df.drop(["operations"], axis=1).reset_index(drop=True),
        ...
        if "grouped" in df.columns:
            ...
            df = pd.concat(
                [
                    df.drop(["grouped"], axis=1).reset_index(drop=True),
    ...
    def safe_to_csv_operations(self, data: List[OperationsByDate], filename: str):
        data_to_save = []
        for item in data:
            ...
            for employee in data:
                for operation in employee["operations"]:
                    if operation and operation.date:
                        ...
                        logger.info(
                            f'Нет данных по операциям для сотрудника {employee["employee_id"]}'
                        )
