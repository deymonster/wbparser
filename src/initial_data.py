...
with SessionLocal() as db:
    with open("processed_data.csv", "r", encoding="utf-8-sig") as file:
        ...
        for row in csv_reader:
            ...
            company = row[2]
            manager = row[3]
            office_area = float(row[4]) if row[4] else 0.0
            rent = float(row[5])
            ...
            existing_office = (
                db.query(OfficeObject).filter_by(office_id=office_id).first()
            )
            if not existing_office:
                ...
                logger.info(
                    f"Office ID {office_object.office_id} with name {office_object.name} was  added"
                )
            else:
